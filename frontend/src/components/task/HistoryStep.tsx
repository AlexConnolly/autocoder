...
export default function HistoryStep({ entry, defaultOpen = false }: Props) {
  ...
  return (
    <div>
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-2 w-full text-left py-1.5 hover:bg-zinc-800/50 rounded px-1 -mx-1 transition-colors"
      >
        <span className="text-zinc-600 text-xs w-3 flex-none">{open ? '▼' : '▶'}</span>
        <span className="text-sm text-zinc-200 flex-1 truncate">{entry.columnName}</span>
      ...
      {open && (
        <div className="mt-1 ml-3 border-l border-zinc-700 pl-3 space-y-3 pb-2">
          {summary && (
          ...
          {entry.content && (
            <details className="group">
              ...
              <div className="mt-2 bg-[#0a0a0b] rounded p-3 font-mono text-[12px] text-zinc-400 whitespace-pre-wrap overflow-x-auto max-h-60 overflow-y-auto leading-relaxed">
                {entry.content}
              </div>
            </details>
          )}
