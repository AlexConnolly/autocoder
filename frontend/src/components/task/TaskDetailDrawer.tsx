...
export default function TaskDetailDrawer({ task, board, liveOutput, contextEntries, onClose, onAnswer, onApprove, onDelete }: Props) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={cn(
        'fixed z-30 bg-surface border-border transition-transform duration-300',
        ...
        {task && (
          <div className="flex flex-col h-full">
            {/* Header */}
            <div className="flex items-start gap-3 p-4 border-b border-border sticky top-0 bg-surface z-10">
              <button
                onClick={onClose}
                className="mt-0.5 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded p-1 transition-colors flex-none"
                aria-label="Close"
              >
                <CloseIcon />
              </button>
              ...
              <div className="flex items-center gap-1 mt-0.5 flex-none">
                {!confirmingDelete ? (
                  <button
                    ...
                    className="text-zinc-500 hover:text-red-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded p-1 transition-colors"
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              {task.status === 'PendingApproval' && (
                <section className="rounded-lg border border-zinc-300/60 dark:border-zinc-600/40 bg-zinc-100/60 dark:bg-zinc-800/30 p-4 space-y-3">
                  <div className="text-xs font-medium text-zinc-500 uppercase tracking-wider">Ready to proceed</div>
                  <p className="text-sm text-zinc-700 dark:text-zinc-300">The agent finished this stage. Start the next stage?</p>
                  <button
              ...
              <section>
                ...
                <dl className="space-y-2 text-sm">
                  ...
                  <Row label="Board"><span className="text-zinc-700 dark:text-zinc-300">{board.name}</span></Row>
                  <Row label="Created"><span className="text-zinc-700 dark:text-zinc-300">{relativeTime(task.createdAt)}</span></Row>
                  <Row label="Updated"><span className="text-zinc-700 dark:text-zinc-300">{relativeTime(task.updatedAt)}</span></Row>
                  {task.errorMessage && (
...
function AnswerPanel({ question, onSubmit }: { question: string; onSubmit: (answer: string) => void }) {
  ...
  return (
    <section className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-4 space-y-3">
      ...
      <p className="text-sm text-zinc-800 dark:text-zinc-200 leading-relaxed">{question}</p>
      <div className="border-t border-amber-500/20 pt-3 space-y-2">
        <textarea
          ...
          rows={3}
          className="w-full bg-[var(--color-bg)] border border-zinc-300 dark:border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-600 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/40 resize-none transition-colors"
        />
